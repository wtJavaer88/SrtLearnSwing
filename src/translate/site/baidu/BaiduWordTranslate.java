...
public class BaiduWordTranslate extends BaiduTranslate implements
        IWordTranslate
{
    ...
    {
        JSONArray jsonArray = getJsonObject().getJSONObject("dict_result")
    ...
    {
        JSONObject exchangeObj = getJsonObject().getJSONObject("dict_result")
                .getJSONObject("simple_means").getJSONObject("exchange");
        WordExchange we = new WordExchange();
        we.setWord_done(("" + exchangeObj.get("word_done")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        we.setWord_er(("" + exchangeObj.get("word_er")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        we.setWord_est(("" + exchangeObj.get("word_est")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        we.setWord_ing(("" + exchangeObj.get("word_ing")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        we.setWord_past(("" + exchangeObj.get("word_past")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        we.setWord_pl(("" + exchangeObj.get("word_pl")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        we.setWord_third(("" + exchangeObj.get("word_third")).replace("[", "")
                .replace("]", "").replace("\"", ""));
        return we;
    }
}
